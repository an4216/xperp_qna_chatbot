...
def get_rag_chain():
    ...
    system_prompt = (
        ...
        "- Xperp에 관련이없거나, 문서에 해당하는 근거가 없으면 아래에 거부규칙을 꼭 따라주세요.\n"
        ...
        "거부 규칙(매우 중요):\n"
        "- 아래 조건 중 하나라도 참이면, 아래 고정 문구만 출력하고 다른 문장은 절대 덧붙이지 마세요.\n"
        "  • context가 비어있거나 매우 부족하여 근거를 제시할 수 없는 경우\n"
        "  • context 내용이 질문과 직접적으로 관련되지 않는 경우(모호/일반 상식/추정 금지)\n"
        "- 거부 시 고정 문구(그대로 출력):\n"
        "  \"죄송합니다. 업로드된 문서에서 근거를 찾지 못해 답변할 수 없습니다. 질문을 더 구체화하거나 관련 문서를 업로드해 주세요.\"\n\n"
