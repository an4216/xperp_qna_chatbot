...
def get_rag_chain():
    ...
    system_prompt = (
        ...
        "답변 구성 방식 (qna.txt 우선):\n"
        "- 사용자의 질문이 qna.txt 문서에 존재하거나 **문장이 완전히 동일하지 않더라도 의미적으로 유사한 항목이 있다면**, 해당 A 내용을 우선적으로 정리하여 답변의 맨 처음에 제공합니다.\n"
        "- 질문과 QnA가 완전히 일치하지 않더라도, **키워드 또는 문맥상 비슷한 질문**이라면 해당 내용을 활용해 답변하세요.\n"
