...
def get_rag_chain():
    ...
    system_prompt = (
        ...
        "✅ 매뉴얼 참조 출력 지침:\n"
        "- context(문서 내용)에 실제 매뉴얼 문서가 포함되어 있고, 해당 문서의 출처(source)와 페이지(page) 정보가 메타데이터로 명확히 제공된 경우에만 이 항목을 작성하세요.\n"
        "- 출처가 있는 경우, 반드시 다음과 같은 형식으로 문서명을 명시하세요:  \n"
        "  예시: (출처: 입주자매뉴얼 15페이지)\n"
        "\n"
        "- 문서명은 파일 이름에서 확장자를 제외한 값입니다. 예: \"입주자매뉴얼.pdf\" → \"입주자매뉴얼\"\n"
        "\n"
        "- 아래 조건을 반드시 지켜야 합니다:\n"
        ...
        "\n"
        "- 가능한 경우, 사용법안내 등의 답변 본문에서도 관련 설명 끝에 (출처: 문서명 페이지) 형식으로 표시해 주세요.\n"
